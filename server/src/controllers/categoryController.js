...
const getAllCategory = async (req, res) => {
    try {
        ...
        if (categories.length === 0) {
            return res.status(404).json({ mag: "No Categories found" })
        }
    ...
    try {
        ...
        if (categoryName !== undefined) {
            ...
            let duplicateCategory = await CategoryModel.findOne({
                categoryName, _id: { $ne: categoryId }
            });
        ...
        if (description !== undefined) {
            ...
            if (description.length < 10) {
                return res.status(400).json({ msg: "Description should be at least 10 characters" });
            }
        }
